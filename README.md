...
*pine* only reacts to X events and messages it receives through a dedicated socket.
...
* Finally, here is a quick example of how to use pinemsg in the sxhkd config:
    ...
    ```
    alt + {h,j,k,l}
        pinemsg focus {left,down,up,right}
...
* When called, *pinemsg* connects to the dedicated socket and sends the arguments that were passed to it. *pine* sleeps until the socket is activated or it receives an X event. This is achieved by using a file descriptor set and select(). This usage of IPC allows for a highly modular setup, where all your keybinds are not tied to a single window manager.
...
* Finally, *pine* is a non-reparenting window manager, which improves simplicity and readability of the project. Reparenting was almost redundant, since *pine* would not draw any title bars or sophisticated window borders by design.
